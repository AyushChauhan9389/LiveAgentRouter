...
export function DeviceNode({ id, data }: NodeProps) {
    ...
    if (type === "fan") {
      ...
      return (
        <svg
          ...
          viewBox="0 0 24 24"
          fill="currentColor"
          className={`w-5 h-5 ${isOn ? "text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.8)]" : "text-cyan-700"}`}
        >
          <path d="M12 2.25a.75.75 0 0 1 .75.75v2.25a.75.75 0 0 1-1.5 0V3a.75.75 0 0 1 .75-.75ZM7.5 12a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM18.894 6.166a.75.75 0 0 0-1.06-1.06l-1.591 1.59a.75.75 0 1 0 1.06 1.061l1.591-1.59ZM21.75 12a.75.75 0 0 1-.75.75h-2.25a.75.75 0 0 1 0-1.5H21a.75.75 0 0 1 .75.75ZM17.834 18.894a.75.75 0 0 0 1.06-1.06l-1.59-1.591a.75.75 0 1 0-1.061 1.06l1.59 1.591ZM12 18a.75.75 0 0 1 .75.75V21a.75.75 0 0 1-1.5 0v-2.25A.75.75 0 0 1 12 18ZM7.758 17.303a.75.75 0 0 0-1.061-1.06l-1.591 1.59a.75.75 0 0 0 1.06 1.061l1.591-1.59ZM6 12a.75.75 0 0 1-.75.75H3a.75.75 0 0 1 0-1.5h2.25A.75.75 0 0 1 6 12ZM6.697 7.757a.75.75 0 0 0 1.06-1.06l-1.59-1.591a.75.75 0 0 0-1.061 1.06l1.59 1.591Z" />
        </svg>
      );
  ...
  return (
    <div className="relative w-56">
      ...
      <div
        className={`relative bg-cyan-50 border-b-4 border-r-4 rounded-lg p-1 transform transition-transform hover:-translate-y-1 hover:shadow-xl border-t border-l group ${isOn ? "border-cyan-600 border-l-cyan-200 border-t-cyan-200" : "border-stone-400 border-l-stone-200 border-t-stone-200"}`}
      >
        <NodeMenu id={id} color="text-cyan-900" />
        <div
          className={`p-3 border rounded transition-colors ${isOn ? "bg-cyan-100 border-cyan-300" : "bg-stone-100 border-stone-200"}`}
        >
          <div className="flex justify-between items-center mb-3">
            <div className="flex items-center gap-2">
              <div
                className={`p-1.5 rounded-full ${isOn ? "bg-cyan-500 text-white shadow-[0_0_10px_rgba(6,182,212,0.5)]" : "bg-stone-300 text-stone-500"}`}
              >
                {getIcon()}
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] font-mono font-bold text-cyan-900 uppercase leading-none">
                  {type}
                </span>
                <span
                  className={`text-[8px] font-bold uppercase tracking-wider ${isOn ? "text-green-600" : "text-stone-500"}`}
                >
                  {isOn ? "ONLINE" : "OFFLINE"}
                </span>
              </div>
            ...
            {/* Toggle Indicator */}
            <div
              className={`w-2 h-2 rounded-full ${isOn ? "bg-green-500 animate-pulse" : "bg-red-400"}`}
            ></div>
          </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-0.5">
                  {[1, 2, 3, 4, 5].map((l) => (
                    <div
                      key={l}
                      className={`w-1.5 h-3 rounded-sm ${l <= ((data.fan_speed as number) || 0) ? "bg-cyan-500" : "bg-cyan-200"}`}
                    ></div>
                  ))}
                </div>
              </div>
            ...
            {type === "AC" && (
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-black text-cyan-600">
                  {(data.temperature as string | number) || "--"}°C
                </span>
              </div>
            )}
